...

{
  ...
  function builtBlockStatement(body) {
    return {
      type: "BlockStatement",
      body: body
    }
...
PostToken             = "post"          !IdentifierPart
PutToken              = "put"           !IdentifierPart
PatchToken            = "patch"         !IdentifierPart
DeleteToken           = "delete"        !IdentifierPart
HeadToken             = "head"          !IdentifierPart
OptionsToken          = "options"       !IdentifierPart
...
HeaderLiteral "header name"
  = head:IdentifierStart tail:HeaderLiteralPart* {
    return head + tail.join('');
  }
...
HeaderValue "header value"
  = !('"' / "\\"/ LineTerminator) head:HeaderValuePart tail:HeaderValuePart* {
    return head + tail.join('');
  }
...
VersionPath
  = !('"' / "\\" / LineTerminator) head:VersionPathPart tail:VersionPathPart* {
    return head + tail.join('');
  }
